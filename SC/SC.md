...
Finding mistakes should be a sure-in for reasons why a code review is important.
Often, we tend to see things as we expect them to be, and not as they really are.
This could not be more true when coding. We have an idea of how the code is supposed to look,
or what that varible is supposed to be called, but perhaps it didn't actually come out that way.
Linter may not even pick up that error, if it is syntaxtually correct! Code reviews offer one line of 
defense against this.

### Polishing strengths

During a code review we have a chance to re-affirm things we did well. When someone points out 
my strengths, that does increase my confidence, as well as raise my moral. Put simply, it
**feels good** when someone points out things we did right. That's important in almost all facets of
interpersonal socio interaction, but I feel especially so in coding environment, where our worst enemy
is usually ourself.

### Finding opportunities for improvement

Arguablly the most import aspect of a code review is finding areas for improvement. Even grandmaster 
coders have some area they could further improve. It could be in logic flow, variable naming, or even
in code readability. There is always room for improvement, and finding such an area gives us a chance
to improve, which further increases our own self validation!
...
Even if all computers were set up exacly the same, it would still be good practice to have consistency
across all distributions. By using a container, you are providing just that. The environment that 
each computer utilitizes when using that app will be exactly the same, every time. This ensures that 
no external variables can disrupt the intended functionality of the app.

### Sterile environment

Adding to the idea of ensuring functionality, sterility of the operating environment is key. There 
are a lot of things going on in the background on each computer, so having a clean slate from which
to load the application does provide security for the running processes of the application. No factors
from that computer's operating system or local environment will impede on the application.
